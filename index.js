  ...
  getActivePolicyCount().then(function(activeCount) {
    return base('Policies').create({
      ...
      bot.sendMessage(chatId,
        ...
        + '💰 Payout: ' + COVERAGE_AMOUNT + ' SOL\n'
        + '⏰ Window: 2 hours\n'
        + '📊 Active: ' + (activeCount + 1) + '/' + MAX_POLICIES + '\n\n'
        + '━━━━━━━━━━━━━━━━━━━━\n'
        + '📖 How your policy works:\n\n'
        + '• You are covered for the next 2 hours\n'
        + '• If the token drops 55%+ and stays\n'
        + '  down for 30 minutes, you get paid\n'
        + '• You must hold the token during the\n'
        + '  2-hour window — selling voids the policy\n'
        + '• Once triggered, you can sell freely\n'
        + '  while we process your payout\n'
        + '━━━━━━━━━━━━━━━━━━━━');
...
bot.onText(/\/payclaim(?:\s+(\S+))?/, function(msg, match) {
  ...
  .then(function(records) {
    ...
    return base('Policies').update(pid, { status: 'PAID' }).then(function() {
    ...
    for (var p = 0; p < policies.length; p++) {
      (function(pol) {
        chain = chain.then(function() {
          ...
          // 2. Check if user still holds the token
          var holdingsCheck = Promise.resolve({ holds: true });
          if (f.wallet_address && f.token_address && HELIUS_KEY) {
            holdingsCheck = checkTokenHolding(f.wallet_address, f.token_address);
          }

          return holdingsCheck.then(function(holding) {
            // If user sold the token, void the policy
            if (!holding.holds && !holding.error) {
              console.log('[Monitor] User sold token, voiding:', polId);
              return base('Policies').update(polId, { status: 'VOID_SOLD' }).then(function() {
                if (f.user_telegram_id) {
                  bot.sendMessage(f.user_telegram_id,
                    '⚠️ Policy voided: ' + polId + '\n\n'
                    + 'You no longer hold the insured token.\n'
                    + 'Selling the token automatically cancels your coverage.\n\n'
                    + 'Buy back in and /buyinsurance again if needed.');
                }
                for (var a = 0; a < ADMIN_IDS.length; a++) {
                  if (ADMIN_IDS[a]) {
                    bot.sendMessage(ADMIN_IDS[a],
                      '🚫 Voided (sold): ' + polId + ' | ' + (f.user_handle || '?'));
                  }
                }
              });
            }

            // 3. User still holds — check price
            return axios.get(
